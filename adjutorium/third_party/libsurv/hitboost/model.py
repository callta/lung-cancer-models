...
from ._hit_core import _global_init, _hit_grads, hit_tdci
from ._utils import _check_data, _check_params, _hit_eval, _print_eval
...
class model(object):
    ...
    def train(
        ...
        if plot:
            ...
            plot_train_curve(
                eval_result["td-CI"], eval_labels, "Time-Dependent C-index"
            )

        return eval_result

    def predict(self, ddata: xgb.DMatrix) -> np.ndarray:
        """
        Prediction method.
    ...
    def predict_survival_function(
        self, ddata: xgb.DMatrix, plot: bool = False
    ) -> np.ndarray:
        """
        Prediction method.
    ...
    def evals(self, ddata: xgb.DMatrix) -> float:
        """
