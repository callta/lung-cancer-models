...
def plot_train_curve(
    L: List, labels: List = ["Learning Curve"], title: str = "Training Curve"
) -> None:
    """
...
def plot_surv_curve(df_survf: pd.DataFrame, title: str = "Survival Curve") -> None:
    """
    Plot survival curve.
    ...
    if isinstance(df_survf, DataFrame):
        ...
        plt.plot(
            np.array([i for i in range(df_survf.shape[1])]), np.transpose(df_survf)
        )
    else:
...
def plot_km_survf(data: pd.DataFrame, t_col: str = "t", e_col: str = "e") -> None:
    """
