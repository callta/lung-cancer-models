...
class ShapPermutationSamplerPlugin(ExplainerPlugin):
    ...
    def plot(self, shap_values: Any) -> None:
        shap.plots.bar(shap_values)

    def explain(
        self, X: pd.DataFrame, max_evals: Union[int, str] = "auto"
    ) -> np.ndarray:
        if self.task_type == "classification":
            out = self.explainer(X, max_evals=max_evals, silent=True)[..., 1]
        else:
            out = self.explainer(X, max_evals=max_evals, silent=True)
