...
class Treatments:
    ...
    def add(self, name: str, cls: Type) -> "Treatments":
        if name in self._plugins:
            raise ValueError(f"Plugin {name} already exists.")

        if not issubclass(cls, TreatmentsPlugin):
            raise ValueError(
                f"Plugin {name} must derive the TreatmentsPlugin interface."
            )
    ...
    def get(self, name: str, *args: Any, **kwargs: Any) -> TreatmentsPlugin:
    ...
    def __getitem__(self, key: str) -> TreatmentsPlugin:
        return self.get(key)

    def reload(self) -> "Treatments":
        self._plugins = {}
        self._load_default_plugins()
        return self
